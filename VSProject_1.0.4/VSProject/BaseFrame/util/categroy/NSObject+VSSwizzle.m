...
@implementation UIView (VSSwizzle)
...
- (void) swizzleAddSubView:(UIView *)view
{
    ...
    if (view == self) {
        ...
        DBLog(@"%@ can't add self.", [self class]);
        return;
    }
    
    if (![view isKindOfClass:[UIView class]]) {
        
        DBLog(@"%@ can't add %@.", [self class],[view class]);
        return;
    }
...
@implementation NSArray (VSSwizzle)
...
- (id) swizzleObjectAtIndex:(int)index
{
    ...
    DBLog(@"Invalid index %d at %@",index, self);
