...
- (NSArray *)segmentViewTitles{
    ...
    if (_segmentViewTitles == nil) {
        _segmentViewTitles = @[@"全部",@"待付款",@"待发货",@"待收货"];
    }
...
- (void)_initTableViews{
    ...
    for (UITableView *tableView in self.contentTaleViews) {
        ...
        [tableView addHeaderWithCallback:^{
            ...
            // @[@"全部",@"待付款",@"待发货",@"待收货"]
            if (index == 0) {
                orderStatus =[NSString stringWithFormat:@"%@,%@,%@,%@,%@", SZ_ORDER_CREATED,SZ_ORDER_PROCESSING,SZ_ORDER_APPROVED,ORDER_SENT,ORDER_COMPLETED ];
            }else if(index == 1){
                orderStatus = SZ_ORDER_CREATED;
            }else if(index == 2){
                orderStatus = [NSString stringWithFormat:@"%@,%@", SZ_ORDER_PROCESSING,SZ_ORDER_APPROVED];
            }else if(index == 3){
                orderStatus = SZ_ORDER_SENT;
            }
            
            
            [weakSelf refreshOrderList:orderStatus type:@"SALES_ORDER_O2O_SALE,SPACE_ORDER_BOOK" page:@"1" row:@"10" dataSource:weakSelf.dataSources[index] tableView:weakTableView];
        }];
        ...
        [tableView addFooterWithCallback:^{
            ...
            [weakSelf refreshOrderList:orderStatus type:@"SALES_ORDER_O2O_SALE,SPACE_ORDER_BOOK" page:[NSString stringWithFormat:@"%zi",page] row:@"10" dataSource:weakSelf.dataSources[index] tableView:weakTableView];
        }];
    }
}
