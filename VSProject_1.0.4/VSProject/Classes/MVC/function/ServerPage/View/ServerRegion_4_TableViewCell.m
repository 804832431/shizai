//
//  ServerRegion_4_TableViewCell.m
//  VSProject
//
...
@interface ServerRegion_4_TableViewCell () <Server_500_375_TopicClickedDelegate, Server_250_375_TopicClickedDelegate>
...
@property (nonatomic, strong) ServerTheme_500_375_View *row2_col1_view;
...
@implementation ServerRegion_4_TableViewCell
...
- (void)layoutSubviews {
    ...
    self.row2_col1_view.frame = CGRectMake(Get750Width(250.0f), self.spaceLineView.frame.origin.y + self.spaceLineView.frame.size.height, Get750Width(500.0f), Get750Width(375.0f));
    
    self.row2_col2_view.frame = CGRectMake(0, self.spaceLineView.frame.origin.y + self.spaceLineView.frame.size.height, Get750Width(250.0f), Get750Width(375.0f));
}
...
- (UIView *)headView {
    ...
    if (!_headView) {
        ...
        _headView.backgroundColor = _Colorhex(0x9eceff);
...
- (ServerTheme_500_375_View *)row2_col1_view {
    
    if (!_row2_col1_view) {
        
        _row2_col1_view = [[ServerTheme_500_375_View alloc] initWithFrame:CGRectMake(Get750Width(250.0f), self.spaceLineView.frame.origin.y + self.spaceLineView.frame.size.height, Get750Width(500.0f), Get750Width(375.0f))];
...
- (ServerTheme_250_375_View *)row2_col2_view {
    ...
    if (!_row2_col2_view) {
        ...
        _row2_col2_view = [[ServerTheme_250_375_View alloc] initWithFrame:CGRectMake(0, self.spaceLineView.frame.origin.y + self.spaceLineView.frame.size.height, Get750Width(250.0f), Get750Width(375.0f))];
...
- (void)setDataSource:(TopicDTO *)topicDTO {
    ...
    [self.row2_col2_view setData:[self.topicDTO.projectList objectAtIndex:2]];
    [self.row2_col1_view setData:[self.topicDTO.projectList objectAtIndex:3]];
}

- (void)topic_250_375_ClickedAction:(TopicProductDTO *)dto {
    
    if (self.serverRgion_4_Block) {
        self.serverRgion_4_Block(dto);
    }
}
