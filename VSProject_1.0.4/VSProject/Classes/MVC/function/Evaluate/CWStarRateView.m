...
#define FOREGROUND_STAR_IMAGE_NAME @"星评2-icon"
#define BACKGROUND_STAR_IMAGE_NAME @"星评-icon-拷贝"
...
- (UIView *)createStarViewWithImage:(NSString *)imageName {
    ...
    CGFloat width = (self.bounds.size.width - 15 * self.numberOfStars)/self.numberOfStars;
    
    for (NSInteger i = 0; i < self.numberOfStars; i ++)
    {
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imageName]];
        imageView.frame = CGRectMake(i * (width + 15) , 0, width, self.bounds.size.height);
