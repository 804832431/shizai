...
@implementation ArrowUpAndDownButton
...
- (void)setTitle:(nullable NSString *)title forState:(UIControlState)state {
    ...
    if (width + 30 < __SCREEN_WIDTH__ / 3) {
        float imgHeadPosition = ((__SCREEN_WIDTH__/3 - width)/2 + width) + 10; //图片和文字留10像素间隔
        
        float imgTailPosition = __SCREEN_WIDTH__/3 - imgHeadPosition - 20;
        
        [self setImageEdgeInsets:UIEdgeInsetsMake(0, imgHeadPosition, 0, imgTailPosition)];
        [self setTitleEdgeInsets:UIEdgeInsetsMake(0, 0, 0, 0)];
    } else {
        //字符过长
        [self setImageEdgeInsets:UIEdgeInsetsMake(0, __SCREEN_WIDTH__/3 - 10, 0, -10)];
