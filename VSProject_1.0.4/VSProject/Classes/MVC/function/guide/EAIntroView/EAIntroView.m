...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        //[modify by Thomas 翻到最后一页不隐藏]－－－start
        self.swipeToExit = NO;
        //[modify by Thomas 翻到最后一页不隐藏]－－－end
...
- (void)buildFooterView {
    self.pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.frame.size.height - self.pageControlY, self.frame.size.width, 30)];
    //modify by Thomas［pagecontrol圆点］－－start
    ...
    //modify by Thomas［pagecontrol圆点］－－end
...
- (void)makePanelVisibleAtIndex:(NSInteger)panelIndex{
    [UIView animateWithDuration:0.3 animations:^{
        for (NSInteger idx = 0; idx < pageViews.count; idx++) {
            if (idx == panelIndex) {
                [pageViews[idx] setAlpha:1.0f];
            }
            else {
