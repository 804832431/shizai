...
- (NSArray *)segmentViewTitles{
    ...
    if (_segmentViewTitles == nil) {
        _segmentViewTitles = @[@"全部",@"待付款",@"待发货",@"待收货",@"待退款"];
    }
    ...
    for (UITableView *tableView in self.contentTaleViews) {
        ...
        [tableView addHeaderWithCallback:^{
            ...
            if (index == 0) {
                ...
                orderStatus = [NSString stringWithFormat:@"%@,%@", ORDER_PROCESSING,ORDER_APPROVED];
            }else if(index == 3){
                orderStatus = ORDER_SENT;
            }else if(index == 4){
                orderStatus = [NSString stringWithFormat:@"%@,%@",RETURN_REQUESTED,RETURN_ACCEPTED];
            }
            
            
            [weakSelf refreshOrderList:orderStatus type:@"SALES_ORDER_O2O_SALE,SALES_ORDER_B2C,SALES_ORDER_O2O_SERVICE_PAY" page:@"1" row:@"10" dataSource:weakSelf.dataSources[index] tableView:weakTableView];
        }];
        ...
        [tableView addFooterWithCallback:^{
            ...
            [weakSelf refreshOrderList:orderStatus type:@"SALES_ORDER_O2O_SALE,SALES_ORDER_B2C,SALES_ORDER_O2O_SERVICE_PAY" page:[NSString stringWithFormat:@"%zi",page] row:@"10" dataSource:weakSelf.dataSources[index] tableView:weakTableView];
        }];
    }
}
...
- (void)refreshOrderList:(NSString *)orderStatus type:(NSString *)orderType page:(NSString *)page row:(NSString *)row dataSource:(NSMutableArray *)datasource tableView:(UITableView *)tableView{
    ...
    NSDictionary *dic = @{@"userLoginId":partyId, //用户id
                          @"orderId":@"",//	订单号
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
    ...
    return [(NSMutableArray *)self.dataSources[index] count];
...
//    if([order.orderHeader.orderTypeId isEqualToString:SZ_SALES_ORDER_O2O_SALE]){
...
//    }
    ...
    //    else if([order.orderHeader.orderTypeId isEqualToString:SZ_SALES_ORDER_O2O_SERVICE_PAY]){
    //        
